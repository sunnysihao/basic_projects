...
def write_json(in_path: str, check_file: str):
    ...
    for file in tqdm(list_files(in_path)):
        ...
        # pcd_path = r"D:\Desktop\BasicProject\毛岩\吉利\JL_upload_file_1012\set5\3d_url"
        ...
        if not os.path.exists(pcd_file):
            ...
            for box in boxs:  # 读取每个3D框数据
                ...
                attrs = box['attrs']
                if not attrs:
                    no_attr_str = f"作业ID:{data_id}-第{frame}帧-{int_id}号框未选择属性"
                    no_attr.append(no_attr_str)
                    continue
                else:
                    if 'occluded' not in attrs.keys():
                        ...
                        occlude = attrs['occluded']
                        truncated = attrs['truncated']

                    if class_type in two_label_class:
                        if 'subname' in attrs.keys():
                            label1 = class_type
                            label2 = attrs['subname']
                        else:
                            no_attr_str = f"作业ID:{data_id}-第{frame}帧-{int_id}号框缺少子类属性"
                            no_attr.append(no_attr_str)
                            continue
                    elif class_type in ['Non-vehicle', 'obstacle']:
                        if 'subname' in attrs.keys():
                            label1 = attrs['subname']
                            label2 = attrs['subname']
                        else:
                            no_attr_str = f"作业ID:{data_id}-第{frame}帧-{int_id}号框缺少子类属性"
                            no_attr.append(no_attr_str)
                ...
                box = {
                    "id": num_id,#目标 id
                    "dimensions": {   #标注的目标真实世界坐标系下的 3d 尺寸
                        "length": nx,
                        "width": ny,
                        "height": nz
                    },
                    ...
                    "name": label1, #目标类型
                    "subname": label2, #子类名
                    "occluded": occlude, #遮挡情况[0~2]
                    "heading": rz, #平扫角标注
                    "truncated": truncated, #截断情况[0~1.0]
                    "num_lidar_points": point_num, #3D 框内点数标注
                    "2dBox": {
                        "camera1": [[], []],
                        "camera2": [[], []]}
                }
        ...
        result_content = {
            "CarNumber": "", #采集车编号
            "Timestamp": os.path.splitext(file_name)[0], #点云对应的时间戳
            "__type__": "annotes",
            "__version__": file_name,
            "pc_name": file_name, #点云文件名
            "camera1": "", #关联相机的图片文件名，项目与 2dBox 一致
            "camera2": "", #关联相机的图片文件名，项目与 2dBox 一致
            "object": boxes,
            " lidar": "middle",
            "size": {
                "height": 1080,
...
if __name__ == "__main__":
    ...
    # check_file = r"D:\Desktop\BasicProject\毛岩\吉利\json_44075_more_20221018175045\JL_upload_file_1012\set5\detection.check.json"
    # result_json_path = r"D:\Desktop\BasicProject\毛岩\吉利\json_44075_more_20221018175045\JL_upload_file_1012\set5"
