            ...
            if box_type == 'bonepoint':
                ...
                for node in nodes:
                    ...
                    if point_x < 0 or point_x > image_w or point_y < 0 or point_y > image_h:
                        ...
                        if not node['code']:
                            empty_str = f"作业id:{data_id}-{int_id}号框未选择标签"
                            ...
                            for pk, pv in point_mapping.items():
                                if point_id in pv:
                                    ...
                                    continue
                            if not point_group:
                                group_err_str = f"作业id:{data_id}-{int_id}号框未编组或编组信息错误"
                ...
                if not box['label']:
                    ...
                    if len(box['label']) == 3:
                        if 'Carriage-Wheel' in box['category'] and "Visible-Scale" in box['category'] and "Direction" in box['category']:
                            ...
                            wheel_err_str = f"作业id:{data_id}-{int_id}号框车轮标签选择错误"
                    ...
                    #         f_label = f"{label}/{label_direction}/{visible_scale}"
                        ...
                        err_str = f"作业id:{data_id}-{int_id}号框标签选择错误"
        ...
        for file in tqdm(list_files(one_path)):
            ...
            for f in fs:
                ...
                if 'Wheel' in label.split('/'):
                    wheel_num += 1
        count_str = f"{path_name}---矩形框(不含车轮框):{rect_num-wheel_num}|点:{point_num}|车轮框:{wheel_num}|多边形:{polygon_num}"
