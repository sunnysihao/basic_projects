...
def write_json(in_path: str):
    label_list = ['Pedestrian', 'Bicycle', 'Car', 'Bus', 'Truck', 'Tricycle', 'Motorcycle']
    border_l = []
    overlap_l = []
    empty_label_l = []
    null_err = []
    ...
    for file in tqdm(list_files(in_path)):
        ...
        # pcd_path = r"D:\Desktop\BasicProject\王满顺\经纬恒润\42621\data_record_2022_05_06_09_42_27\data_record_2022_05_06_09_42_27\data_record_2022_05_06_09_42_27\point_cloud\pcd\LIDAR_UPPER_CLOUD\3d_url"
        ...
        for box in boxs:  # 读取每个3D框数据
            ...
            job_id = box['cBy']
            attr_label = box['classType']
            if not x or not y or not z or not nx or not ny or not nz:
                null_str = f"作业ID:{batch_id} - 第{frame_number}帧 - 创建人{job_id} - {int_id}号框有null值\n"
                ...
                if not attr_label:
                    label = "null"
                    empty_err = f"作业ID:{batch_id} - 第{frame_number}帧 - 创建人{job_id} - {int_id}号框无标签信息\n"
                    empty_label_l.append(empty_err)

                else:
                    if attr_label not in label_list:
                        label = label_chinese_mapping[attr_label]
                    else:
                        label = attr_label
                ...
                if point_num < wide:
                    border_err = f"作业ID:{batch_id} - 第{frame_number}帧:{pcd_name} - 创建人{job_id} - {int_id}号框点数不符合标注规范-点数{point_num}\n"
        ...
        if lap:  # 如果有框重叠
            overlap_err = f"作业ID:{batch_id} - 第{frame_number}帧有框重叠\n"
    ...
    border_file = os.path.join(in_path, 'border_errors.txt')
    with open(border_file, 'w', encoding='utf-8') as bf:
        bf.writelines(border_l)
    overlap_file = os.path.join(in_path, 'overlap_errors.txt')
    with open(overlap_file, 'w', encoding='utf-8') as of:
        of.writelines(overlap_l)
    empty_file = os.path.join(in_path, 'empty_label_errors.txt')
    with open(empty_file, 'w', encoding='utf-8') as ef:
        ef.writelines(empty_label_l)
    count_file = os.path.join(in_path, 'count_box.txt')
    with open(count_file, 'w', encoding='utf-8') as cf:
        cf.writelines(f"共计{count_box} 个框")
    null_file = os.path.join(in_path, 'has_null_position.txt')
    with open(null_file, 'w', encoding='utf-8') as nf:
        nf.writelines(null_err)
...
if __name__ == "__main__":
    ...
    # result_json_path = r"C:\Users\EDY\Downloads\42621一帧一结果\data_record_2022_05_06_09_42_27"
