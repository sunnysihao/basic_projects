...
def write_json(in_path: str):
    ...
    for file in tqdm(list_files(in_path)):
        ...
        #
        # pcd_path = r"D:\Desktop\BasicProject\王满顺\经纬恒润\pcd上传数据\data_record_2022_05_11_13_29_46\data_record_2022_05_11_13_29_46\data_record_2022_05_11_13_29_46\point_cloud\pcd\LIDAR_UPPER_CLOUD\3d_url"
        ...
        for box in boxs:  # 读取每个3D框数据
            ...
            if not attr_label:
                ...
                if '断裂' in attr_label:
                    broken_err = f"作业ID:{batch_id} - 第{frame_number}帧 - 创建人{job_id} - {int_id}号框存在断裂，pcd文件名为：{file_name}\n"
                    broken_l.append(broken_err)
                    if len(attr_label) == 1:
                        label = "only_has_broken"
                        only_broken_err = f"作业ID:{batch_id} - 第{frame_number}帧 - 创建人{job_id} - {int_id}号框只有断裂标签\n"
            ...
            if point_num < wide:
                border_err = f"作业ID:{batch_id} - 第{frame_number}帧 - 创建人{job_id} - {int_id}号框点数不符合标注规范\n"
    ...
    broken_file = os.path.join(in_path, 'broken_errors.txt')
    with open(broken_file, 'w', encoding='utf-8') as kf:
        kf.writelines(broken_l)
    only_broken_file = os.path.join(in_path, 'only_has_broken_errors.txt')
...
if __name__ == "__main__":
    ...
    # result_json_path = r"C:\Users\EDY\Downloads\json_42289_93212_20220804173426"
