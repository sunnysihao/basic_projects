...
def json2xml(json_dir, xml_dir):
    ...
    for file in tqdm(list_files(json_dir, '.json')):
        ...
        trans = doc.createElement('transpose')
        root.appendChild(trans)
        trans_text = doc.createTextNode(str(transpose))
        ...
        for box in boxes:
            ...
            sup_cate_text = doc.createTextNode(super_cat)
            sup_cate.appendChild(sup_cate_text)

            _name = doc.createElement('name')
            _object.appendChild(_name)
            name_text = doc.createTextNode(cat_name)
